...
`atlas-chain-index-lab` is centered on blockchain tooling work in C. The technical target is: Implement a C blockchain tooling project for index format conversion, using round-trip fixtures and lossless normalization checks.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 6.
